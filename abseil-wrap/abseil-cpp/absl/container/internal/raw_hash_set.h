...
class raw_hash_set {
 ...
 public:
  ...
  /*need some elements to put on the stack frame
  jump to some location in code. (this location can be variable)
  *f -> change where we jump. 

  (*f)(key);
  *f = &find1;
  object.f(key);
  rehash happens -> *f = &find2;
  object.f(key);

  template <class K = key_type>
  iterator find1(const key_arg<K>& key) {
    return find(key, hash_ref1()(key));
  }

  template <class K = key_type>
  iterator find2(const key_arg<K>& key) {
    return find(key, hash_ref2()(key));
  } */
  ...
  uint64_t num_h2_pass() const { return num_h2_pass_; }
  uint64_t num_visited_group() const { return num_visited_group_; }
 ...
 private:
  ...
  uint64_t num_visited_group_ = 0; // number of visited groups during search
  uint64_t num_h2_pass_ = 0;       // number of times where h2 code matches with key
...
public:
  // moved to public group for dummy hashing	
