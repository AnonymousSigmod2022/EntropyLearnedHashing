...
res_std_wyhash_final=""
res_std_wyhash_sub8_final=""
for data_size in 1000 50000
do
	num_probes=$data_size
	for fract_successful in 0 1
	do
		res_temp1=$(cat results/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful.res | grep "std_wyhash -")
        res_temp2=$(echo $res_temp1 | cut -d "-" -f 2 | cut -d " " -f 2)
        res_std_wyhash_final+=" ${res_temp2}"

		res_temp1=$(cat results/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful.res | grep "std_wyhash_sub8 -")
        res_temp2=$(echo $res_temp1 | cut -d "-" -f 2 | cut -d " " -f 2)
        res_std_wyhash_sub8_final+=" ${res_temp2}"
	done
done
...
echo $res_std_wyhash_final
echo $res_std_wyhash_sub8_final
