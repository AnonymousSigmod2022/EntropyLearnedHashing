...
res_swiss_default_final=""
res_swiss_wyhash_final=""
res_swiss_wyhash_sub8_final=""
for data_size in 1000 50000
do
	num_probes=$data_size
	for fract_successful in 0
	do
		res_temp1=$(cat results_insertion_time/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful.res | grep "swiss_default -")
		res_temp2=$(echo $res_temp1 | cut -d "-" -f 3 | sed 's/ //g')
	    res_swiss_default_final+=" ${res_temp2}"

		res_temp1=$(cat results_insertion_time/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful.res | grep "swiss_wyhash -")
        res_temp2=$(echo $res_temp1 | cut -d "-" -f 3 | sed 's/ //g')
        res_swiss_wyhash_final+=" ${res_temp2}"

		res_temp1=$(cat results_insertion_time/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful.res | grep "swiss_wyhash_sub8 -")
        res_temp2=$(echo $res_temp1 | cut -d "-" -f 3 | sed 's/ //g')
