...
mkdir -p results_insertion_time
...
do
	...
	do
		./set_options_insertion_time.sh "./cleanData/$dataset" $data_size $num_probes $fract_successful $byteLocations1B $byteLocations4B $byteLocations8B
		numactl --physcpubind=1 --membind=1 ./benchmarker > results_insertion_time/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful.res 2>&1
	done
done
