...
std::string formPartialStringDefaultToFull(const std::string& inputString, const vector<int>& positions) {
...
unsigned int getQuantileLength(vector<string>& inputVec, double quantile) {
    vector<int> lengths; 
    std::transform(inputVec.begin(), inputVec.end(), std::back_inserter(lengths), [](const std::string& inputString) { return inputString.size();});
    std::sort(lengths.begin(),lengths.end());
    unsigned int posInVec = quantile * lengths.size();
    return lengths[posInVec];
}
...
double entropy_estimators::greedyRenyiSelectorStopK(
    ...
    unsigned int ninety_quantile_len = getQuantileLength(inputVec, 0.1);
    ...
    for (int i = 0; i < numPositions; i++) {
        ...
        cout << "i: " << i << "," << endl;
        time.start();
        if (returnPositions.size() == maxLengthString / sizeof(T)) {
            break;
        }
        pair<int, double> nextPosAndEntropy;
        nextPosAndEntropy = chooseBestPositionRenyi<T>(maxLengthString, inputVec, returnPositions);
