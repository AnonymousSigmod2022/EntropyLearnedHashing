...
do
	...
	do
		...
		do
			./set_options_mt.sh "./cleanData/$dataset" $data_size $num_probes $fract_successful $byteLocations1B $byteLocations4B $byteLocations8B $nthd
			numactl --cpunodebind=1 --membind=1 ./benchmarker > results_mt/$dataset\_$data_size\_keys_$num_probes\_probes_$fract_successful\_fract_successful_$nthd\_nthd.res 2>&1
		done
	done
done
